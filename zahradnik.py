...
Tento záhon využívá fakt, že česnek uvolní místo v červenci, což otevírá prostor pro 'druhou směnu' letní zeleniny.
...
def main():
    ...
    if "weather" in st.secrets:
        ...
        if w:
            ...
            c4.image(f"http://openweathermap.org/img/wn/{icon}.png", width=50)
            
            if temp < 5.0:
                st.error(f"❄️ **MRAZÍK:** {temp}°C! Chraň citlivé plodiny.")
            elif temp > 25.0 and hum < 40:
                st.warning("☀️ **SUCHO:** Zkontroluj Blumaty.")
    ...
    with tab3:
        ...
        if not df_real.empty:
            st.divider()
            target = st.selectbox("Akce pro:", df_real.index, format_func=lambda x: f"{df_real.loc[x, 'Plodina']} ({df_real.loc[x, 'Pozice']})")
            h, s, d = st.columns(3)
            if h.button("💧 Hnojit"):
